...
// {en-US} Get the projected location of [point] from [camera]
...
class Triangle {
	...
	public:
		...
		Location3 getA() const {
			return *a;
		}

		Location3 getB() const {
			return *b;
		}

		Location3 getC() const {
			return *c;
		}
		...
		std::vector<cv::Point3d> toOpenCVPointArray() const {
			std::vector<cv::Point3d> points;

			points.push_back(a->toOpenCVPoint());
			points.push_back(b->toOpenCVPoint());
			points.push_back(c->toOpenCVPoint());

			return points;
		}
};

// {en-US} Render [cube] to [out] from the perspective of [camera].
void renderCube(cv::Mat& out, const Camera3& camera, const Cube& cube) {
	...
	std::vector<cv::Point> opencvPoints;

	for (int i = 0; i < 12; i++) {
		auto face = faces[i];

		opencvPoints.push_back(getProjectedLocation(face.getA(), camera).toPoint());
		opencvPoints.push_back(getProjectedLocation(face.getB(), camera).toPoint());
		opencvPoints.push_back(getProjectedLocation(face.getC(), camera).toPoint());

		cv::fillPoly(out, opencvPoints, red);
